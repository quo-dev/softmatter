const gulp = require( 'gulp' ),
    fancylog = require( 'fancy-log' ),
    browserSync = require( 'browser-sync' ),
    server = browserSync.create(),
    dev_url = 'http://softmatter-wp.local';


/**
 * Define all source paths
 */

var paths = {
  styles: {
    src: './assets/scss/main-style/**/*.scss',
    vendor: './assets/scss/vendor/*.scss',
    variable: './assets/scss/variable/**/*.scss',
    dest: './assets/css'
  },
  scripts: {
    src: './assets/js/**/*.js',
    dest: './assets/js'
  },
  view: {
    src: './**/*.php',
  }
};


/**
 * Webpack compilation: http://webpack.js.org, https://github.com/shama/webpack-stream#usage-with-gulp-watch
 *
 * build_js()
 */

function build_js() {
  // const compiler = require( 'webpack' ),
  //     webpackStream = require( 'webpack-stream' );
  console.log('start build js here');
  return gulp.src( paths.scripts.src )
  // .pipe(
  //     webpackStream( {
  //           config: require( './webpack.config.js' )
  //         },
  //         compiler
  //     )
  // )
  // .pipe(
  //     gulp.dest( paths.scripts.dest )
  // )
      .pipe(
          server.stream() // Browser Reload
      );
}
function build_template() {
  return gulp.src( paths.view.src )
      .pipe(
          server.stream() // Browser Reload
      );
}



/**
 * SASS-CSS compilation: https://www.npmjs.com/package/gulp-sass
 *
 * build_css()
 */

function build_css() {
  const sass = require( 'gulp-sass' )( require( 'sass' ) ),
      // postcss = require( 'gulp-postcss' ),
      // sourcemaps = require( 'gulp-sourcemaps' ),
      autoprefixer = require( 'autoprefixer' ),
      cssnano = require( 'cssnano' );

  const plugins = [
    autoprefixer(),
    cssnano(),
  ];
  return gulp.src( paths.styles.src )
  // .pipe(
  //     sourcemaps.init()
  // )
      .pipe(sass().on( 'error', sass.logError ))
      // .pipe(
      //     postcss( plugins )
      // )
      // .pipe(
      //     sourcemaps.write( './' )
      // )
      .pipe(
          gulp.dest( paths.styles.dest )
      )
      .pipe(
          server.stream() // Browser Reload
      );
}

function build_vendor_css() {
  console.log('building bootstrap...');
  const sass = require( 'gulp-sass' )( require( 'sass' ) );
  return gulp.src( paths.styles.vendor )

      .pipe(sass().on( 'error', sass.logError ))

      .pipe(
          gulp.dest( paths.styles.dest )
      )
      .pipe(
          server.stream() // Browser Reload
      );
}


gulp.task( 'watch',
    function () {
      // Modify "dev_url" constant and uncomment "server.init()" to use browser sync
      server.init({
        proxy: dev_url,
      } );

      gulp.watch( paths.scripts.src, build_js );
      gulp.watch( paths.view.src, build_template );
      gulp.watch( [ paths.styles.src, './assets/scss/main-style/**/*.scss' ], build_css );
      gulp.watch( [ paths.styles.variable,paths.styles.vendor], build_css );
      gulp.watch( [ paths.styles.variable,paths.styles.vendor ], build_vendor_css);
    }
);
