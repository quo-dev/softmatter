...
function the_link($link, $cssClass='', $icon=false){
	if(isset($link['url'])){
		...
		echo '<a  href="'.$link['url'].'" class="'. $cssClass .'" target="'.$link['target'].'" data-wow-duration="1.4s" data-wow-offset="10">'.$link['title'].' '.$icon_img.'</a>';
	}
}
...
function is_current_url($url) {
    ...
    if (str_contains($actual_link, $url) || (str_contains($url, 'news') && is_single()) || $current_url == $url) {
        return true;
    }
...
function get_link_breadcrumb($breadcrumb) {
    $link = '<a href="'. get_home_url() .'">Home</a><span> › </span>';
    if ($breadcrumb) {
        foreach ($breadcrumb as $key => $post_id) {
            $link .= '<a href="'. get_permalink($post_id) .'">' . get_the_title($post_id) . '</a>';
